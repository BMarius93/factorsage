...
| Repeated BUY levels   | each BUY level fires **at most once per position lifecycle**. A later, higher level tops the position up toward its target; a level already satisfied is marked fired without a trade                                             |
...
| Monthly contribution  | `first-eligible-trading-day-of-month@1`: the first simulated trading date of each calendar month, **except** the run's first simulated date, which already receives the initial capital                                           |

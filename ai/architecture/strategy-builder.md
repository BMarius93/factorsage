...
| `DUPLICATE_EXIT_RULE`                     | two **semantically** identical Exit Rules in one FINAL EXIT; identity is the rule's canonical logic, never its id, and is **order-insensitive across the ANDed Conditions** since AND is commutative. Validation-only: it never touches the order-preserving fingerprints that key `definitionHash` and a Monitor's latch | `strategies.md` § FINAL EXIT     |

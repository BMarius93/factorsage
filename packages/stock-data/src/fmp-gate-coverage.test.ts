...
describe("shared FMP provider gate", () => {
  ...
  it("is constructed by exactly the known composition roots", () => {
    ...
    //
    // Three entry points here are not long-running services: the benchmark prewarm CLI, the stock
    // resync CLI and the alternative-data ingest CLI. They are on the list for exactly the reason the
    // list exists — a thirty-year index backfill, a re-verification of thirty-odd securities' whole
